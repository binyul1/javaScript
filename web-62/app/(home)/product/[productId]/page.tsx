...
export default async function ProductDetail({
  ...
  const productDetail = await ProductService.getProductById(param.productId);
  return (
    <>
        ...
        <div className="max-w-7xl px-4 mx-auto 2xl:px-0">
          <div className="lg:grid lg:grid-cols-2 lg:gap-8 xl:gap-16">
            <div className="shrink-0 max-w-md lg:max-w-lg mx-auto">
              <img className="w-full " src={productDetail.thumbnail} alt="" />
            </div>
            ...
            <div className="mt-6 sm:mt-8 lg:mt-0">
                ...
                <div className="flex items-center gap-2 mt-2 sm:mt-0">
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((_, index) => (
                      <svg
                        key={index}
                        className={`size-4 ${index >= Math.ceil(productDetail.rating) ? "text-gray-300" : "text-yellow-400"}`}
                        aria-hidden="true"
                  ...
                  <p className="text-sm font-medium leading-none text-gray-500 ">
                    {Math.ceil(productDetail.rating)}
                  </p>
                  <a
                    href="#"
              ...
              <p className="mb-6 text-gray-500 ">{productDetail.description}</p>
            </div>
          </div>
        </div>
      </section>
